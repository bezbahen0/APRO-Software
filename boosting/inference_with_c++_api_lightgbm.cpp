...
int main(int argc, char **argv)
{
    ...
    for(int j=0; j < numFeats; j++)
    {
        const auto nbins = dset->Feature(j)->bin_mapper()->num_bin();
        std::cout << "Feat " << numFeats << std::endl;
        std::cout << "   " << dset->Feature(j)->bin_mapper()->BinToValue(0) << " ";
        std::cout << "   " << dset->Feature(j)->bin_mapper()->BinToValue(nbins-2) << " ";
    ...
    LightGBM::ObjectiveFunction objConfig;
